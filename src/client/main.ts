...
function captureInteractiveState(): PreservedInteractiveState {
    const activeElement = document.activeElement;
    let activeInputKey: string | null = null;
    const inputs: PreservedInputState[] = [];

    getPreservableInputs().forEach((input, index) => {
    ...
    return {
        activeInputKey,
        inputs,
        scrollContainers: getPreservableScrollContainers().map((element, index) => ({
            key: getPreservedScrollKey(element, index),
            scrollLeft: element.scrollLeft,
            scrollTop: element.scrollTop,
        })),
        panels: getPreservablePanels().map((element) => ({
...
function restoreInteractiveState(state: PreservedInteractiveState) {
    ...
    getPreservableScrollContainers().forEach((element, index) => {
    ...
    getPreservablePanels().forEach((element) => {
...
function getPreservableInputs(): Array<HTMLInputElement | HTMLTextAreaElement> {
    return Array.from(APP_EL.querySelectorAll("input, textarea"))
        .filter((input): input is HTMLInputElement | HTMLTextAreaElement => (
            input instanceof HTMLInputElement || input instanceof HTMLTextAreaElement
        ));
}

function getPreservableScrollContainers(): HTMLElement[] {
    return Array.from(APP_EL.querySelectorAll(".hand-list"))
        .filter((element): element is HTMLElement => element instanceof HTMLElement);
}

function getPreservablePanels(): HTMLElement[] {
    return Array.from(APP_EL.querySelectorAll("[data-panel]"))
        .filter((element): element is HTMLElement => element instanceof HTMLElement);
}
...
function updateInputCounter(input: HTMLInputElement | HTMLTextAreaElement) {
    const action = input.getAttribute("data-action");
    if (action === "create-card-input") {
        updateCharCounter(input as HTMLInputElement, "create-card");
    } else if (action === "create-conversation-input") {
        updateCharCounter(input as HTMLInputElement, "create-conversation");
    }
}
...
type PreservedInteractiveState = {
